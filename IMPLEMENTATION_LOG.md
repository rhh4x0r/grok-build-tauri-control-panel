...
## 2026-09-21 — Server-first projects, phase 3: server projects in the app

- `bomb_app::remote`: paired servers (`ServerConfig` saved in the local settings table, holding nothing secret), a device key per server in the login keychain with a private-file fallback (`BOMB_KEY_DIR` forces files for tests), pairing from a link, and one reconnecting connection per server (1–30 s backoff) that keeps an event cursor, drops this Mac's own mirrored prompts, and asks the app to rebuild open server threads when the server demands a resync.
- Server folders are namespaced on this side as `bomb-server://<server>/<path>`, so everything keyed by project root keeps working. `core_router::Core` is `Local(AppState) | Remote(RemoteCore)` with methods mirroring the services; `core_for_root`, `AppModel::core_of_thread` and `core_of_workspace` pick the owner. Converted: thread list, workspaces, projects, snapshot/hydrate (server threads also restore open approvals), start, wait, send, stop, approvals and allow rules, approval mode, effort, rename, remove, review/standing, project overview and status, merge request, merged check, close feature, checkpoint/update, branch list, folder readiness, backends and sign-in status for the project in view. Lists from each server are cached so a brief disconnect does not empty the sidebar.
- Core additions: `rpc` methods `branches`, `thread_setup_check`, `thread_setup_initialize`, `create_project` (an empty Git project under the core's `~/projects`); serde derives on `BackendInfo`, `SessionIdResponse`, `Readiness`, `BranchChoices`.
- UI: Settings → Servers (pair with a link, connection state, new project on the server, make a pairing link for another Mac or, as admin, another person, list and remove devices, unpair with confirmation; the admin note says plainly that the server owner can read everything on it). Sidebar projects on a server wear a server mark tinted by connection state; "Add project" becomes a menu offering each server first, then this Mac. On server threads the Terminal, Dev sidebar, Changes panel, Finder reveal and open-folder entries are hidden until phase 5.
- Validation: `remote::tests::a_paired_mac_runs_a_project_on_the_server_and_survives_a_dropped_connection` runs the real client path against a loopback gateway and core: pair (bad link refused), connect, create a project (slugged, under the core's home), namespaced project/thread/workspace/event paths, folder readiness and overview, start a thread, send a prompt, receive the reply without an echo of our own prompt, snapshot, an unsupported Changes action refused in plain words, then device removal on the server takes this Mac offline with a reason and later calls fail fast. Serial suites all passing: bomb_app 20, bomb_core 73 (+4 ignored), bomb_proto 2, bomb_link 2, bomb_server 5. `bombd up` was started for real with a scratch home: it printed a pairing link, wrote `identity.json` and `registry.json` with mode 0600, and removed its socket on stop.
- Not verified: the new screens in the running app (no screen access from the agent shell), and any real server. The model picker for a server project lists that server's backends only after it connects.


## 2026-09-21 — Server-first projects, phase 4: projects on and off the server with Git

- `bomb_core::services::project_sync`: history travels as `git bundle`s (HEAD plus every branch, minus whatever the other side already has). `init_from_bundle` makes a normal clone with local branches and no leftover remote. `import_bundle` never resets or overwrites: incoming branches are staged under `refs/bomb/incoming/`, then each branch is created, fast-forwarded (moving the files too when it is checked out and that folder has no unsaved tracked edits), left alone when this side is ahead, reported as blocked by unsaved edits, or reported as diverged with the incoming copy kept for a merge. History that does not connect is refused in plain words. `clone_url` accepts only https/SSH Git addresses.
- Protocol: numbered binary streams both ways. The client picks the stream id; `upload` sends a file then names the stream in its request; `download` registers the stream before asking. In `bombd`, uploads land in a private transfer folder (4 at once, 4 GB cap), are attached to a request as `bundle_path` only by the server (any client-sent `bundle_path` is stripped), and are deleted after use; exports are answered with size and SHA-256, streamed, then deleted. New methods: `branch_tips`, `import_bundle`, `import_project`, `export_bundle`, `clone_project`; all but the last two require `root` to be one of that person's registered projects.
- App: `remote::sync` (`send_to_server`, `download_copy`, `sync`, with downloaded bundles verified by size and SHA-256 and scratch files always removed) and `AppModel` actions plus a saved link between a Mac folder and its server project. Project page: "Put on a server", "Download a copy to this Mac" (into Documents/BombCode), or "Sync" once linked. Add project menu: "Clone from GitHub onto <server>…".
- Validation: real-Git core test covers first upload, no-op sync, incremental return with files moving, a branch with nothing new not being sent, divergence kept on both sides without touching local work, unsaved edits blocking then succeeding after cleanup, and unrelated history refused. The loopback client test now also sends a Mac project up through the gateway, syncs with nothing to do, picks up a server commit, sends offline work back, downloads a fresh copy, checks no bundle is left behind, and confirms the server refuses a folder that is not the person's project and ignores a client-named file. Serial suites all passing (bomb_core 75 +4 ignored, bomb_app 20, bomb_server 5, bomb_proto 2, bomb_link 2). UI not exercised; LFS and submodules are not handled or warned about yet.


## 2026-09-21 — Server-first projects, phase 5: server projects feel local

- Terminal: `TerminalSession` gained a server-backed variant (`remote` + `feed`, ops sent out through a callback) and a tapped local spawn for the server side. `bombd` keeps per-connection live streams: `terminal_open` starts a login shell in one of the person's folders and relays its raw output; client chunks on that stream are keystrokes; `terminal_resize`; ending the stream kills the shell. The app's terminal panel opens one automatically for server threads.
- Preview: `forward_open` connects only to the server's own loopback and relays bytes both ways. In the app, starting the dev server on a server project starts it there (`start_dev_server` over RPC), opens a free local port that forwards to it, and points the preview at `http://127.0.0.1:<local>`; stopping or unpairing closes the port. Local status polling no longer overwrites a server preview's status.
- Files: `list_dir` and `read_file` (2 MB cap, `.git` hidden), plus dev server start/stop/status, are confined by `own_path`: the folder must be one of the person's registered projects or thread folders, paths must be relative without `..`, and symlinks are resolved before the containment check. The file tree lists server folders through it.
- Terminal and Dev sidebar buttons are back on server threads. Still hidden there: the Changes panel (diff and staging), Finder reveal and open-folder.
- Validation: `bomb_server` socket test covers listing, reading, `..`/absolute/symlink/foreign-folder refusals, a terminal round trip with resize and close, a forwarded HTTP exchange, and a closed port failing fast. The loopback client test now also types into a server terminal and sees the screen and the shell's exit, refuses a terminal in a folder on an unknown server, loads a loopback-only dev server through a forwarded local port twice, and confirms the port closes with the guard. Serial suites all passing. None of it was seen in the running app; limits of 32 live streams per connection.


## 2026-09-21 — Server-first projects, phase 6: several people on one server

- `bomb_server::admin`: the only privileged surface. `bombd admin <verb> <account>` accepts four verbs (`create-user`, `lock-user`, `start-core`, `stop-core`) and one argument that must match a server-generated `bc-` + 4–16 lowercase alphanumerics name; commands run with a cleared environment and a fixed PATH. New accounts get a private 0700 home, a nologin shell and no extra groups. The plan of commands is a pure function, tested on the Mac. The gateway reaches it through a `Privileged` interface: `Sudo` on a shared server (`bombd gateway --shared`), `Unavailable` on a one-person install.
- Gateway, admin only: `gateway.invite_person` creates the account, starts that person's core and returns a pairing link bound to it; `gateway.lock_person` stops their core, locks the account, deletes their open links and devices and cuts their live connections; nobody can lock themselves.
- `bombd core` accepts a systemd-activated socket (fd 3). `deploy/server/` holds `bombd-gateway.service`, the per-person `bombd-core@.socket` and `bombd-core@.service`, a tmpfiles entry, a sudoers line and a README. Isolation design: systemd owns each person's socket as `<person>:bombd 0660` inside `/run/bombd` (`root:bombd 0750`); people are not in group `bombd`, so they cannot reach one another's sockets, while the unprivileged gateway can. Core units add `NoNewPrivileges`, `PrivateTmp`, `ProtectSystem=strict` with only the person's home writable, and memory, CPU and task limits.
- App: Settings → Servers shows admins "Invite a person" and a people list with a confirmed "Lock out", with the plain statement that the server's owner can read everyone's data; everyone gets "Link for another Mac of mine".
- Validation: helper name and plan tests; a gateway test with a stand-in for root covers invite (account created then core started, non-admin, socket path), the invited person pairing and being refused admin actions, self-lock refused, lock-out cutting access and links, and the one-person refusal. bomb_server 9 and bomb_app 20 tests passing.
- NOT verified: nothing in this phase has run on Linux. The systemd units, sudoers line, tmpfiles entry, socket activation and `useradd`/`usermod` flags are unexercised; treat `deploy/server` as a first draft to try on a throwaway VPS.


## 2026-09-21 — Server-first projects, phase 7: installer and sign-in on the server

- Sign-in: for a project on a server, "Sign in" asks the server for that provider's login command (`login_command`) and runs it in a terminal on the server, shown in a dialog on the Mac, so the person opens the link or pastes the code in their own browser. The Mac's login files are never sent. Closing the dialog rechecks sign-in status. This reuses the phase 5 terminal, so it works for Claude and Codex, which need a TTY.
- Installer (`remote::install`, Settings → Servers → "Set up a new server"): uses the system `ssh`/`scp` in batch mode with `--` before the target, so keys, SSH config and known_hosts behave as in Terminal and the app never hangs on a question. It checks `uname -sm`, picks `bombd-linux-x86_64` or `-aarch64` from `$BOMBD_BINARIES` or `~/.bombcode/server`, uploads it, and runs a fixed script that installs to `~/.local/bin`, writes a systemd user service running `bombd up`, tries `loginctl enable-linger` and `ufw allow`, reports missing `git`/`node`/agent CLIs, and returns a pairing link, which the app then pairs with. The SSH login and the public address are validated against strict character sets, so neither can become an SSH option or shell text; the script interpolates only those validated values. No root is needed for a one-person install.
...
## 2026-09-21 — First real server: Max's Omarchy box

- Installed with the app's own setup script (run by hand over SSH to see errors directly): upload, `~/.local/bin/bombd`, a systemd user service, linger, pairing link. Worked first time. Two things surfaced and were fixed in the installer: the service now inherits the login shell's PATH (the agent CLIs there live behind mise shims), and the firewall step also tries firewalld. The port had to be opened by hand because sudo needs a password.
- Pairing, connecting, creating a server project and a thread all worked. Model refresh failed with "rpc error -32603: internal error": the Claude ACP adapter's npx-fetched binary was truncated (35 MB of 234 MB) and crashed on load, and the Codex adapter's npx copy lacked its platform package. Clearing both npx cache entries fixed them. Grok showed only two models because it was not signed in on the server. Fable 5.1 appears there as `claude-fable-5-1[1m]`.
- Quitting the Mac app for 10 s while a turn ran: the server kept going (no cancellations, agent processes alive) and the thread caught up on reopen. Gap: the reopened thread looked idle until its next event, because saved rows carry no "in progress" state. Fixed: a server thread whose status is running or waiting is shown as working straight after hydrating (`Thread::resume_in_progress`).
- Follow-ups noted, not done: the "-32603 internal error" message must name the cause; the adapters should verify downloads before running them; the sign-in status shown for a server should make "not signed in on the server" obvious.

- Status line: a running tool no longer reads as "Quiet · tool" after 25 s. While a tool runs the line shows its clock ("Running · cargo test · 1m 20s", from 5 s on); only after five silent minutes does it say "Still running · … — taking a while". "Quiet" is kept for real silence with no tool in flight, now after 45 s. Presence gained `tool_started_at`; tests updated and extended.
- Explainer cost: it called the model every 5 s while a thread had new lines, with Grok's default (top) model. It now speaks only at a moment (a tool finishing or failing, a message, the turn ending, an approval) and at most every 20 s, with a once-a-minute word during long log-only stretches; approvals still interrupt at once (`narration_due`, tested). When no narrator is configured and Claude is signed in, it defaults to Haiku; a chosen provider/model in Settings is respected. Not measured against real usage yet.

- One project, not two: a Mac copy of a server project now lists under that project in the sidebar (`AppModel::sidebar_root`), its threads mixed in with the server's, each row marked with a small server or laptop icon; either copy in view highlights the same entry. The project page says in words where the copy in view lives and whether the other exists. Sync now first saves a checkpoint in every idle server thread with unsaved edits (`checkpoint_project`), reports how many, says plainly when there was nothing to sync, and puts the Mac copy on the branch of the thread in view when the copy is clean. Prompted by Max finding an "empty" Mac copy: the server agent's work had never been committed, so all branches sat at the first commit and Sync silently moved nothing.

- "Sync with this Mac's copy" read as a direction. The button is now "Sync both ways", its tooltip says exactly what moves where, and the project page shows the state before you press it ("Copies: Server is ahead on bomb/tetris · This Mac is ahead on main · Changed on both sides: …", or "Both copies have the same saved work."). Computed by `remote::sync::compare` from both sides' branch tips plus a new `has_commits` RPC, refreshed with the project overview and after each sync.

- Device keys moved out of the macOS keychain into a private file, `~/.grok/control-panel/servers/<id>.key.json` (folder 0700, file 0600, written atomically), the same model as an SSH key. The keychain asked for the login password on every rebuild, and it does not exist on Linux or Windows. A key an earlier build stored in the keychain is read back once through `security find-generic-password` and moved to the file, so the paired Mac stays paired. The `security-framework` dependency is gone from the app.


## 2026-09-21 — Threads that build on other threads

- Research on gitflow, GitHub flow and stacked branches (Graphite, `git town`) came out as: keep one `main`, one branch per thread, merge not rebase, and add the one thing people actually do with stacks — start a thread on top of another thread's unmerged work. Nothing in the UI says rebase, restack, force push or detached HEAD.
- Starting a thread: the "From <branch>" dropdown is now a plain choice, "Start from main" (default) or "Continue from <Thread> · N saved changes not in main yet" for each open thread whose branch is ahead; other branches list underneath. The row then says "Builds on <Thread> · merge that thread first, then this one", and a toast repeats it once when the thread starts. A thread's ⋯ menu has "New thread from here" which pre-selects it (`AppModel::new_thread_from`).
- Core (`services/workspaces.rs`): a child is an open thread whose `base_ref` is another open thread's branch (`parent_of`, `stack_links`). `merge_request` refuses a child with "Merge “X” first". When a parent's work lands (`close_feature`, or `restack` after a merge that keeps the parent open) each idle, clean child gets `main` merged in and its `base_ref` re-pointed at `main`, so its next merge goes there; a busy or dirty child is skipped and picked up next time; a conflicted merge is left in place as `NeedsHand(files)`. Real-Git test covers refuse → parent lands → child brought up to date → child merges.
- App: `stack_links` and `restack` RPCs, `AppModel::stacks` refreshed with the project overview. Board cards show "Builds on X · N newer changes there" and "A, B built on this · when this merges, they are brought up to date"; a child's merge buttons become a disabled "Merge X first". After a parent's merge turn ends the app checks `is_merged`, restacks, toasts each outcome, and on a conflict opens the child's chat and asks its agent to finish the merge in progress.

- Opus 5.5 and the Codex 6 models were missing from the picker. The lists come from the ACP adapters at session start (Claude's from its SDK, Codex's from OpenAI's server), and the app launched them with `npx --yes <package>`: a bare name makes npx reuse whatever it cached first, so the Mac was on `claude-agent-acp 0.79` / `codex-acp 1.12` while 0.81 / 1.13 advertise the new models. Adapters now launch as `<package>@latest` with `--prefer-online`; npx still uses its cache when offline. The same resolver runs in `bombd`, so the server picks this up with its next deploy.

- A thread started on a project's Mac copy was missing from the sidebar: the entry is shared by both copies, but its rows were taken only from workspaces whose root was the entry's own (`sidebar.rs`). Rows now come from either copy, each with its laptop or server mark. The project page gained a "Threads on this Mac" / "Threads on <server>" panel listing the other copy's open threads with agent, branch and Open chat, and one line saying where they run and that saved work crosses over on sync.
- The thread was also called "Attached file open from path": a message that was only an attachment got its name from the note the app appends for files. `prompt_slug` now ignores that note and, when nothing else was typed, names the thread after the file ("Metacanon Conversation Compilation"). A failed attachment upload to the server now keeps the prompt in the composer like a failed start does, instead of dropping it.
