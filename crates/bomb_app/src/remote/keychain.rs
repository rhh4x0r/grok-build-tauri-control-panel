...
pub fn load(server: &str) -> Option<Identity> {
    if let Some(identity) = serde_json::from_slice(&std::fs::read(path(server)?).ok()?).ok() { return Some(identity); }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if out.status.success() {
            ...
            let identity: Identity = serde_json::from_slice(&bytes).or_else(|_| serde_json::from_str(&hex)).ok()?;
