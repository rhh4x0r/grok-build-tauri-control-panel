...
pub async fn discover(state: &AppState, backend: Backend, config: &GrokConfig) -> BackendInfo {
    ...
    let mut info = BackendInfo { id: backend.key().into(), display_name: descriptor.display_name.into(), available: false, via: None, reason: None,
        default_model: String::new(), models: Vec::new(), model_names: HashMap::new(), model_descriptions: HashMap::new(), model_error: None, supports_headless: descriptor.supports_headless };
    ...
    let result: Result<ModelCatalog, String> = async {
        if let Some(catalog) = state.registry.backend_model_catalog(backend).await { return Ok(catalog); }
        if backend == Backend::Grok {
            ...
            return Ok(ModelCatalog { current: models.iter().find(|(_,default)| *default).map(|(id,_)|id.clone()),
        ...
        AcpClient::discover_models(launch).await.map_err(|e|e.to_string())
    }.await;
    match result {
